<script>
    ...
    // Загрузка медиа-файлов из локальных assets
    async function loadMediaFiles() {
        try {
            ...
            if (dialogue.backgroundImage) {
                try {
                    backgroundImageUrl = await loadAsset(dialogue.backgroundImage)
                } catch (err) {
                    console.warn('Background image not found:', dialogue.backgroundImage)
                }
            }
            ...
            if (dialogue.characterImage) {
                try {
                    characterImageUrl = await loadAsset(dialogue.characterImage)
                } catch (err) {
                    console.warn('Character image not found:', dialogue.characterImage)
                }
            }
    ...
    // Функция для загрузки ассетов
    async function loadAsset(filename) {
        // Пробуем разные пути для ассетов
        const paths = [
            `/assets/${filename}`,
            `/assets/character_images/${filename}`,
            `/assets/other_characters/${filename}`,
        ...
        <div class="background-media">
            ...
            {:else}
                <img
                        ...
                        on:load={() => isLoadingMedia = false}
                        on:error={() => errorMedia = 'Ошибка загрузки фона'}
                />
            {/if}
        </div>
    {/if}
        ...
        <div class="character-media">
            ...
            {:else}
                <img
                        ...
                        on:load={() => isLoadingMedia = false}
                        on:error={() => errorMedia = 'Ошибка загрузки персонажа'}
                />
            {/if}
        </div>
    {/if}
        ...
        {#if dialogue.options && dialogue.options.length > 0}
            <div class="options-container">
                {#each dialogue.options as option (option.text)}
                    <button
                            class="option-button"
                            on:click={() => handleOptionSelect(option.nextDialogueId)}
                    >
                        {option.text}
                    </button>
                {/each}
